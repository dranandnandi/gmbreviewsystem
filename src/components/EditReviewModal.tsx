...
interface EditReviewModalProps {
  ...
  review: Review | null;
  onClose: () => void;
  onSave: (id: string, updates: Partial<Pick<Review, 'patientName' | 'appointmentDate' | 'contactNumber' | 'treatment' | 'notes'>>) => Promise<void>;
}

export const EditReviewModal: React.FC<EditReviewModalProps> = ({ isOpen, review, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (review) {
      setForm({
        ...
        appointmentDate: review.appointmentDate.split('T')[0],
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input name="treatment" value={form.treatment} onChange={handleChange} className="mt-1 w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
            </div>
